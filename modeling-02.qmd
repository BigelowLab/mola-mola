...
Here we modify our first modeling workflow to produce a model for each month. In the [previous workflow](modeling-01.html) we produced one model covering observations covering all times; we then applied the model to the various months. 
...
preds = lapply(names(models),
  function(mon){
    ix = which(month.abb %in% mon)
    predict(models[[mon]], dplyr::slice(x, time, ix, drop), type = "cloglog")
  }) 
preds = do.call(c, append(preds, list(along = list(time = date_sequence))))
```
...
plot(preds |> st_to_180(), hook = plot_coast)
```
